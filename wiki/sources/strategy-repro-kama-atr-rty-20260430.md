...
| 品种 | 数据范围 | Sharpe (默认参数) | Sharpe (OAT优化) | Trades | MaxDD | 评级 |
|------|---------|------------------|-----------------|--------|-------|------|
| RTY.CME | 2024-06 ~ 2025-12 | -3.98 | **1.78** | 206 | -4.78% | 🟢 Green |
| ES.CME | 2024-06 ~ 2025-12 | 0.80 | **1.75** | 332 | -18.6% | 🟢 Green |
| NQ.CME | 2024-06 ~ 2025-12 | -2.50 | - | 346 | -232% | 🔴 Red |
| CL.NYMEX | 2024-05 ~ 2025-12 | -0.06 | 超时 | 840 | -1.1% | 🟡 Yellow |
| GC.COMEX | 2024-05 ~ 2025-12 | -0.36 | - | 570 | -31.2% | 🔴 Red |

**说明**：
- RTY 和 ES 表现优秀（Sharpe > 1.7），策略在股指期货上有效
- NQ 波动过大，默认参数导致巨额回撤（-232%），不适合此策略
- CL 和 GC（商品期货）表现不佳，策略逻辑可能不适用于商品趋势
- YM 无 1 分钟数据，无法测试
...
| 品种类型 | 适配性 | 说明 |
|---------|--------|------|
| 股指期货（RTY/ES） | ✅ 高 | KAMA+ATR 趋势跟踪适合股指日内波动 |
| 科技股指数（NQ） | ❌ 低 | 波动过大，止损参数需大幅调整 |
| 能源（CL） | ⚠️ 中 | 数据量大但趋势特征不同 |
| 贵金属（GC） | ❌ 低 | 黄金趋势与股指不同，参数不适配 |
