# VIX Futures Trading Strategy: Backtest and Example Analysis

**Source**: [[2026-04-04-VIX-Futures-Trading-Strategy:-Backtest-and-Example-Analysis]] | [Unknown](https://quantifiedstrategies.substack.com/p/vix-futures-trading-strategy-backtest-f98)
**Date**: 2026-04-04
**Tags**: #article #substack #strategy #backtest

## One-Sentence Summary

> VIX Futures Trading Strategy: Backtest and Example Analysis
QuantifiedStrategies.com
May 28, 2024
1
Share
A VIX futures strategy refers to the methods and techniques you can use to trade VIX futures, ...

## Key Insights

1. **原文来源**: [Unknown](https://quantifiedstrategies.substack.com/p/vix-futures-trading-strategy-backtest-f98)

## Full Content

VIX Futures Trading Strategy: Backtest and Example Analysis
QuantifiedStrategies.com
May 28, 2024
1
Share
A VIX futures strategy refers to the methods and techniques you can use to trade VIX futures, which are financial derivative products that represent a contract to trade a specified unit of the index at a pre-agreed price on a future date.
In this post, we answer some questions about the VIX futures strategy and we also make a backtest.
We backtest the following trading rules:
VIX must set a new 20-day high,
The five-day RSI must be higher than 70 (for the VIX,
If both rules above are correct, we buy S&P 500 (SPY),
We sell at the close when the close is higher than yesterday’s high.
The equity curve of the strategy looks like the image shown below.
You can find more info about this trading strategy here:
https://www.quantifiedstrategies.com/vix-futures-trading-strategy/
1
Share
Previous
Next

---

*由 Substack Strategy Tracker 自动抓取*

---

*Imported from Substack on 2026-04-25*
