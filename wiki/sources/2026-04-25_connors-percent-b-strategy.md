...
**方法**: strategy_factory 管线 -> 手动修复生成代码 -> BacktestingEngine 直接回测

| 指标 | 结果 |
|------|------|
| 策略 | LarryConnorsBStrategy |
| 品种 | SPY.SMART |
| 时间范围 | 1993-01-01 ~ 2026-04-25 |
| K线类型 | 日线 |
| Sharpe Ratio | 0.47 |
| 总收益率 | 4.03% |
| 年化收益率 | 0.19% |
| 交易次数 | 72 |
| 参数 | sma_period=200, bb_period=20, bb_std_dev=2.0 |

**说明**: 回测交易次数（72）与原文（63）接近，验证了策略逻辑的正确性。Sharpe 0.47 偏低，但这是该策略的设计使然——仅在4%的时间里持仓，赚取小而频繁的均值回归收益。

**与 RSI2MR 对比**: 两个都是 Connors 风格的均值回归策略（RSI2 用 RSI < 5 做信号，%B 用 %B < 0 做信号），入场逻辑非常相似。RSI2MR 在 QQQ/SPY 上的交易频率更高（100-600笔），Sharpe 也更高（0.8-1.2）。

**复现过程发现的问题**:
1. strategy_factory 的 YAML 中 `percent_b` 指标不是 vnpy ArrayManager 的标准方法，需要手动实现 `(close - lower) / (upper - lower)`
2. 代码生成器存在已知 Bug：数值比较 `1[-1]`、参数名冲突 `sma_200_period vs sma_period`、`am.boll()` 返回 2 个值而非 3 个
3. 所有 bug 均属 skill 文档中已记录的「已知 Bug 模式」
